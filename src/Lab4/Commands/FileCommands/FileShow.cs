...
public class FileShow : ICommandWithParamsFlags
{
    ...
    public void Execute()
    {
        ...
        string varstring = _param[2];
        if (!Path.IsPathRooted(varstring))
        {
            varstring = Path.Combine(State.Head, varstring);
        }

        string res = State.Receiver.ShowFile(varstring);
